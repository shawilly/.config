...
local inc = -1
local timer = nil
local color = string.format("#%x", vim.api.nvim_get_hl_by_name("@alpha.title", true).foreground or 16777215)
local background = string.format("#%x", vim.api.nvim_get_hl_by_name("Normal", true).background or 16777215)
...
M.update_colors = function()
	vim.api.nvim_set_hl(0, "@alpha.title", { link = "Structure", default = false })
	vim.api.nvim_set_hl(0, "@alpha.header", { fg = vim.api.nvim_get_hl(0, { name = "String" }).fg, bold = true })
	vim.api.nvim_set_hl(0, "@alpha.footer", { fg = vim.api.nvim_get_hl(0, { name = "Constant" }).fg, italic = true })
	background = string.format("#%x", vim.api.nvim_get_hl_by_name("Normal", true).background or 0)
	color = string.format("#%x", vim.api.nvim_get_hl_by_name("@alpha.title", true).foreground or 0)
...
M.in_mathzone = function(_, matched_trigger)
	...
	while current_node do
		...
		elseif current_node:type() == "generic_command" then
			local cmd_name = vim.treesitter.get_node_text(current_node:named_child(0), 0, {})
